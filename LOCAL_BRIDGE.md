...
> **精准修补的确定性编辑（v0.9.13，仅 Claude Code）**：选「精准修补」并发送给 Claude Code 时，Agent 不重写整页，只给出要改的若干处（替换文本 / 设置样式），由本地组件**确定性应用**到源文件副本，改动严格限定在你圈选的范围。默认先弹出「将要应用的精确修改」清单待你确认（可在 Side Panel「高级选项」改为「直接应用，事后审阅」）；确认后才生成候选并新开页签，候选页会高亮实际改动区域。Agent 给不出可用编辑时自动回退为生成完整候选。审计证据同上（`runs/<run-id>/` 内含 `source.html` 快照、`edits.json`、`candidate.html`、manifest）。
