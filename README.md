...
团队批注绑定账号身份，只有作者本人可以编辑或删除自己的评论。已有团队的账号登录后会回到上次活跃的团队；加入其他团队由用户主动发起。你可以随时在账号面板修改显示名称，修改后会同步显示给团队成员。
...
- **v1.0.1（当前版本）**：匿名使用统计扩展——新增协作与任务链路指标（回复/看到他人评论/查看与确认修改计划/应用候选/切换团队等），仍完全匿名、不含任何内容与账号信息；发送任务统计口径修正（计划预跑不再计入）。
- **v1.0.0**：1.0 里程碑。新增团队成员显示名称自定义（改名即同步给成员，不会被 Google/飞书资料覆盖）；本地 HTML 文件体验全面稳定——更新扩展后无需刷新即可评论、打开侧栏即可选中评论、本地文档彻底不参与团队同步（登录后本地评论仅自己可见且始终可编辑）；发送按钮与「逐条编辑预览」说明文案更准确（预览仅 Claude Code 支持）。
- **v0.9.17**：修复升级后部分用户页面批注组件未注入，以及本地 HTML 页面打开侧栏后无法评论的问题；评论卡片新增作者头像和「你」标记，修复 Agent 发送菜单无法展开；安全加固——登录限流防密码爆破、团队治理审计日志、服务端内容清洗与标准安全响应头、gitleaks 密钥扫描 CI。
