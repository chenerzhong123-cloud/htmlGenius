...
- **v1.0.0（当前版本）**：1.0 里程碑。新增团队成员显示名称自定义（改名即同步给成员，不会被 Google/飞书资料覆盖）；本地 HTML 文件体验全面稳定——更新扩展后无需刷新即可评论、打开侧栏即可选中评论、本地文档彻底不参与团队同步（登录后本地评论仅自己可见且始终可编辑）；发送按钮与「逐条编辑预览」说明文案更准确（预览仅 Claude Code 支持）。
