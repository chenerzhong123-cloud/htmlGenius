...
- **v1.0.4（当前版本）**：修复邮箱验证码倒计时每秒重建输入框，导致内容被清空、失焦和无法提交的问题；同时补齐 Chrome 商店版扩展 ID 的服务端跨域允许。
- **v1.0.3**：扩展以全新品牌名「PageTack – Web Annotation」发布；受邀成员可用邀请码 + 邮箱验证码免密码加入团队；团队成员可一键汇总当前网站全部未解决评论，复制为按页面分组的 AI 修改 Prompt。
