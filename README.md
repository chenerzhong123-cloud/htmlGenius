...
### v0.9.13（2026-07-31 · 当前版本）
- 「精准修补」改为确定性编辑：Claude 只给出要改的若干处，由扩展精确应用，改动严格限定在你圈选的范围。默认先给你看「将要改什么」的清单、确认后再生成候选（可在高级选项改为直接应用）；候选页会高亮实际改动的区域。目前仅「精准修补」+ Claude Code 支持，其余仍生成完整候选。
...
- **确定性精确编辑仅限特定组合**：改动严格限定圈选区的「确定性编辑」目前只支持「精准修补」+ Claude Code；其他 Agent 或模式仍生成完整候选（Agent 给不出精确编辑时也会自动回退为完整候选）。
