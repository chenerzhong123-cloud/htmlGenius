...
> 在任意网页上划词评论，挑选本次要处理的意见，一键打包成带精确定位的修改契约——复制给 AI，或直接交给本机的 Claude Code 产出一份可回退的候选版本。让 AI 生成的 HTML 越改越准。
...
- **交给本机 Claude Code（v0.7.1 / Night Pack A）**：把契约交给你本机已登录的 Claude Code，产出一份**新的候选 HTML（只写候选，绝不覆盖原文件）**，打开后评论自动重定位，并在侧边栏显示「原文件未修改 / 重锚统计」等只读证据。仅 macOS + Chrome + Node 20+，详见 [`docs/LOCAL_BRIDGE.md`](docs/LOCAL_BRIDGE.md)。
...
3. 重复，把所有想讨论的地方都标出来。
4. 点底部「**整理评论，创建编辑任务**」→ 勾选本次要处理的评论 → 「继续」→ 在契约里补充目标 / 保护项 → 「**复制 Prompt**」粘给 AI，或「**发送给 Claude Code**」交给本机 Agent。

> 在一条评论上悬停，可以**回复**、**编辑**或**删除**（编辑 / 删除仅限作者本人；删除会连同其下所有回复一起删）。

## 最近更新

### v0.7.1 / v0.7.2 / Night Pack A（2026-07-20 · 当前版本）
...
- **本机 Agent 闭环需准备**：需 `claude auth login` 并安装 host（仅 macOS + Node 20+）；候选闭环的真实 Claude 运行需消耗你本机额度，自动化测试用假 CLI 覆盖，真实 smoke 待你手动验证。
