...
### v0.9.13（2026-07-31）
- 「精准修补」改为确定性编辑：Claude 只给出要改的若干处，由扩展精确应用，改动严格限定在你圈选的范围。默认先给你看「将要改什么」的清单、确认后再生成候选（可在高级选项改为直接应用）。当时仅「精准修补」+ Claude Code 支持。
...
- **确定性精确编辑的支持范围**：改动严格限定圈选区的「确定性编辑」目前支持「精准修补」+ **Claude Code / Codex**；**Copilot 适配已完成但真机验证后开放**，其余模式仍生成完整候选（Agent 给不出精确编辑时也会自动回退为完整候选）。
