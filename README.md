...
未登录时，批注保存在浏览器本地。主动使用团队协同时，团队批注会同步到 htmlGenius 自有服务，以便成员实时查看。“复制全站评论给 AI”只会在用户点击时读取当前团队、当前网站域名下的未解决评论，并写入本机剪贴板；不会读取其他团队或其他网站的数据。不会收集浏览历史，不接广告；统计仅为匿名功能使用事件（不含页面内容与账号信息，详见隐私政策的「匿名使用统计」）。
