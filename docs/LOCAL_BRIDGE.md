# htmlGenius Local Bridge (v0.9 · macOS · Claude Code / Codex / GitHub Copilot)

让 Side Panel 把一份「修改契约」**一键交给你本机已登录的 AI Agent**，产出**只写候选、绝不覆盖原文件**的新版本（`原名V1.N.html`），或先生成可审阅的**修改计划**。

> 仅 macOS + Chrome + Node `^20.19.0 || >=22.12.0`。host 名 provider-neutral（`com.htmlgenius.local_bridge`），三个 provider 复用同一个 host。
...
> ⚠️ **发行状态**：npm 包 `@htmlgenius/bridge` 尚未发布。正式发布前，Connection Center 处于**开发态**（显著标注「仅开发环境」），给出的是仓库内开发命令（见 §4）；正式渠道只保留「复制 Prompt」降级路径，不会给出必然失败的命令。发布后切换为：
> ```bash
> npx --yes @htmlgenius/bridge@0.9.0 setup --json --scope user --extension-id <你的扩展ID>
> ```
