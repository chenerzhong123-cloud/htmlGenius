# 已废弃：v0.7 Codex Local Bridge 实现 Spec

> **不要按本文施工。** Codex adapter 的有效编号与唯一实施依据已改为 [v0.8 Codex App Server Adapter Spec](2026-07-21-v0.8-codex-app-server-adapter-spec.md)。
>
> 原因：此前版本号与 Claude handoff / candidate 路线重叠，且本文将 candidate 协议、版本存储和 provider 接入混在旧的实施顺序中。保留本文只作历史参考，不能与 v0.8 一起执行。

> 日期：2026-07-18  
> 前置版本：v0.6.1 Change Contract、v0.6.2 Artifact Version Reconciliation 已完成。  
> 路线更新（2026-07-19）：本 spec 保留为 **Codex adapter** 施工依据，但其实施顺序后移；先完成 `docs/2026-07-19-v0.7.1-claude-code-handoff-spec.md` 的 Claude Code handoff，并复用其 Native Host、session ownership 与 task bundle 基础。
...
## 13. v0.7 Codex adapter 完成定义与后续边界
...
Claude Code handoff 已独立为优先实施的 v0.7.1；其后是 Claude candidate execution、review/promote，再接入本 Codex adapter。candidate 与 source 的 diff、局部/全局变更审查、接受后安全提升为正式文件、对精确修补的结构化操作验证，以及可交互 approval 都仍是后续里程碑。不要把这些未解决能力提前伪装为“自动安全编辑”。
