...
test("JS:状态矩阵五分支齐全(§5.2)", () => {
  assert.match(sp, /rc === "OS_UNSUPPORTED"/);
  assert.match(sp, /rc === "BRIDGE_PROTOCOL_TOO_NEW"/);
  assert.match(sp, /bs === "install_required" \|\| rc === "BRIDGE_NOT_INSTALLED"/);
  assert.match(sp, /rc === "BRIDGE_PROTOCOL_TOO_OLD" \|\| rc === "BRIDGE_FILES_CORRUPT"/);
  assert.match(sp, /bs === "repair_required"/);
  assert.match(sp, /bs === "ready" && readyCount > 0/);
  assert.match(sp, /t\("conn\.titleConnected"\)\.replace\("\{n\}"/);
});
...
test("JS:ready+有 provider 默认折叠;Copy Prompt 降级提示常驻", () => {
  assert.match(sp, /function connAutoCollapsed/);
  assert.match(sp, /connSetPermanent\(t\("conn\.promptStillAvailable"\)/);
});
