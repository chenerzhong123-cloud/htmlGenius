...
test("v0.9:版本单一来源 — 无 0.8.1 漂移,扩展版本取 getManifest", () => {
  ...
  assert.match(bg, /TARGET_BRIDGE_VERSION = "0\.9\.4"/);
});
...
test("v0.9:bootstrap 固定模板 — 变量仅 id/版本(严格校验),不得出现 latest;production 发行态,dev 模板保留为回退", () => {
  ...
  // dev 模板字符串仍保留在源码中(回退路径),显著标注「仅开发环境」
